...
async function buildPdf(branch: string, rows: ProductRankRow[]) {
  ...
  pdf.setAuthor("ICB Technologies Costa Rica");
  ...
  const startPage = (continuation = false) => {
    ...
    page.drawText("ICB Technologies Costa Rica", { x: margin, y: page.getHeight() - 32, size: 10, font: bold, color: rgb(1, 1, 1) });
  ...
  try {
    ...
    return new NextResponse(responseBody, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="ICB-top-40-${safeFilename(branch)}-${today()}.${extension}"`,
